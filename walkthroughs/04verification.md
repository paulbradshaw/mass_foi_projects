...
Once you've extracted the data you need to check that it's been extracted accurately. The most common issues will be missing data and data that's been stored in the wrong place. 

For best results combine three methods of verification:

1. “Eyeball” tests: manual checks for known risks, vulnerabilities and issues
2. Analysis tests: calculations such as totals and value counts to cross-check
3. AI tests: document comparison and repeat of above tests

Create a checklist for each and work through it systematically for each extracted dataset (you can [find a template here](https://docs.google.com/document/d/1uQZ6dpAIpPwABLkLotBiQucXXFye7X48Q-woA3QBBLU/edit?usp=sharing)):
...
*If original PDF includes totals/uses pivot table/sequential ID numbers

## AI test checklist:

Upload both documents and ask it to
✔ Redo the same checks from eyeball and analysis tests
✔ Ask to suggest other checks that could be performed 

## Prompt templates for checking data

The following are examples of prompts that might be used to check extracted data against the original file.
